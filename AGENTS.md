...
- `R/shape_bezier.R`, `R/shape_bezier_ribbon.R`, `R/curve_bezier.R`,
  ...
  immediately after `shape_bezier.R` (and `shape_bezier_ribbon.R`,
  itself collated right after `shape_bezier.R` since it depends on that
  file’s `bezier_curve_points()`) since it shares `shape_bezier.R`’s
  ...
  no dependency on `drawable` and loads first. `curve_raw.R`/
...
- Every exported function’s roxygen block carries an `@family` tag
  ...
  `@family fill helpers` for every `fill_*()` constructor. **Any new
  drawable or fill helper needs the matching `@family` tag added
  alongside its `@export`.**
