...
(`circle()`, `blob()`, `ribbon()`, `twist()`) that can be composed into a `sketch()`
and rendered with `draw()`.
...
A `blob()` is a circle whose radius is perturbed by simplex noise. Here six
