...
test_that("a zero-width twist's path is exactly the straight backbone", {
  ...
  # width = 0 collapses the path to a straight line regardless of
  # path_distortion's smooth/seed
...
test_that("shape_twist is reproducible for a given path_distortion seed, and varies across seeds", {
  t1 <- shape_twist(width = 1, path_distortion = noise_bridge(seed = 7L))
  t2 <- shape_twist(width = 1, path_distortion = noise_bridge(seed = 7L))
  t3 <- shape_twist(width = 1, path_distortion = noise_bridge(seed = 8L))
  expect_identical(t1@points, t2@points)
  expect_false(isTRUE(all.equal(t1@points@x, t3@points@x)))
})

test_that("shape_twist's width modulation is reproducible for a given distortion seed", {
  t1 <- shape_twist(width = 1, distortion = noise_field(seed = 11L))
  t2 <- shape_twist(width = 1, distortion = noise_field(seed = 11L))
  t3 <- shape_twist(width = 1, distortion = noise_field(seed = 12L))
  expect_identical(t1@points, t2@points)
  expect_false(isTRUE(all.equal(t1@points@x, t3@points@x)))
})
...
test_that("shape_twist accepts path_distortion with smooth = 0 (no smoothing passes)", {
  expect_no_error(shape_twist(path_distortion = noise_bridge(smooth = 0)))
})
