test_that("style()'s default fill is unchanged (\"black\")", {
  expect_identical(fv(style()@fill), "black")
})

test_that("style()'s fill is a fill object", {
  expect_true(S7::S7_inherits(style()@fill, fill))
  expect_true(S7::S7_inherits(style(fill = "red")@fill, fill))
})

test_that("style() accepts a fill_*() pattern/gradient object", {
  expect_true(inherits(fv(style(fill = fill_hatch())@fill), "GridPattern"))
  expect_true(inherits(fv(style(fill = fill_gradient(type = "radial"))@fill), "GridPattern"))
})

test_that("style() still accepts a plain colour string", {
  expect_identical(fv(style(fill = "red")@fill), "red")
})
...
test_that("drawable constructors forward fill_*() outputs to style() unchanged", {
  ...
  expect_true(inherits(fv(b@style@fill), "GridPattern"))
})
