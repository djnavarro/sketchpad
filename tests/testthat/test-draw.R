...
test_that("draw() renders a solid-fill drawable without error", {
  local_null_device()
  expect_no_error(draw(circle(fill = fill_solid("red"))))
})

test_that("draw() renders a fill_hatch()-filled drawable without error", {
  local_null_device()
  expect_no_error(draw(circle(fill = fill_hatch())))
})

test_that("draw() renders a fill_gradient()-filled drawable without error", {
  local_null_device()
  expect_no_error(draw(blob(seed = 1L, fill = fill_gradient(type = "radial"))))
})

test_that("draw() renders a sketch mixing solid and pattern fills without error", {
  local_null_device()
  s <- sketch() +
    circle(fill = fill_solid("blue")) +
    circle(x = 3, fill = fill_crosshatch())
