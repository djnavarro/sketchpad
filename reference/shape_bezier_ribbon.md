...
`shape_bezier_ribbon()`: each argument may be a vector, recycled against
the others via
...
one `shape_bezier_ribbon()` per recycled row, rather than a single
drawable.
